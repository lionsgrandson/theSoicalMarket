...
def handle_hire_save(sender, instance, created, **kwargs):
    ...
    """
    if created:
        # New proposal received — notify influencer
        _dispatch("PROPOSAL_SENT", {
            "hire_id": instance.id,
    ...
    # For updates — only fire on actual state changes, not every save
    try:
