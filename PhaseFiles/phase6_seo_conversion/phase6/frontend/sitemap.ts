// app/sitemap.ts
// Generates /sitemap.xml automatically — Next.js reads this at build time.
// No configuration needed. Just drop this file in the app/ folder.
...
async function getInfluencerIds(): Promise<string[]> {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}user_service/get_all_influencers/?page_size=200`,
      { next: { revalidate: 3600 } }
    );
    if (!res.ok) return [];
    const data = await res.json();
    const results = data?.data?.results ?? data?.data ?? [];
    return results.map((u: { id: string | number }) => String(u.id));
  } catch {
    return [];
  }
}
...
async function getBrandIds(): Promise<string[]> {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}user_service/get_all_brands/`,
      { next: { revalidate: 3600 } }
    );
    if (!res.ok) return [];
    const data = await res.json();
